  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">Discover perspectives and generate content</p>
      </div>

      {/* Start Workflow */}
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Start Content Generation</h2>
        <div className="flex gap-3">
          <input
            type="text"
            placeholder="Enter a topic or leave empty for auto-discovery..."
            value={topicQuery}
      ...
      {workflow && (
        <>
          ...
          {workflow.hot_topics.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Discovered Hot Topics</h2>
                ...
                {workflow.hot_topics.map((topic, i) => (
                  <div key={i} className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <span className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded">Score: {topic.composite_score?.toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {workflow.perspectives.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Discovered Perspectives</h2>
              <div className="space-y-3">
                {workflow.perspectives.map((p, i) => (
                  <div key={i} className="p-4 bg-amber-50 rounded-lg border border-amber-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-500">Confidence: {p.confidence?.toFixed(2)}</span>
                    </div>
          ...
          {workflow.adapted_draft && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Generated Draft</h2>
              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-gray-500">Style Match: {workflow.adapted_draft.style_match_score?.toFixed(2)}</span>
                </div>
          ...
          {workflow.citations.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Source Citations</h2>
          ...
          {workflow.evaluation && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Quality Evaluation</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <MetricCard label="Retrieval Relevance" value={workflow.evaluation.retrieval_relevance} />
                <MetricCard label="Perspective Quality" value={workflow.evaluation.perspective_quality} />
                <MetricCard label="Hallucination Score" value={workflow.evaluation.hallucination_score} invert />
                <MetricCard label="Overall Score" value={workflow.evaluation.overall_score} />
              </div>
            </div>
          )}
